...
```
用户配置 MindOS → Agent 读写知识库产生价值 → 用户写 Skill/模板 → 分享 Skill/配置 → 他人发现 MindOS → 安装
                                                                          ↑                                    |
                                                                          └────────────────────────────────────┘
```
...
| 载体 | 传播机制 | 为什么有效 |
|------|---------|-----------|
| **Agent 配置文件** | 用户分享 CLAUDE.md / .cursorrules，里面写了 MindOS 连接配置 | 开发者爱分享 dotfiles，每份配置都是免费广告 |
...
| 动作 | 具体怎么做 | 验证标准 |
|------|-----------|---------|
| 1:1 跟进 | 主动私聊已安装的用户，问"卡在哪了" | 10+ 人完成 Agent 首次读写 |
...
| **工作流 demo** | 录屏：CLI 或 Agent 读写同一知识库 | 想试但不知道怎么用的人 | YouTube / B站 / GitHub |
