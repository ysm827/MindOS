...
MindOS 内置 10 个渲染器插件，每个插件在匹配到特定文件名/扩展名时自动激活，将纯文本/JSON 渲染为交互式 UI。
...
| [CSV Views](csv-views.md) | `*.csv`（排除 TODO） | 数据可视化 | Yes |
| [Config Panel](config-panel.md) | `CONFIG.json` | 配置编辑 | Yes |
...
| [Agent Inspector](agent-inspector.md) | `*.agent-log.json` | 日志分析 | No |
| [Diff Viewer](diff-viewer.md) | `*Agent-Diff*.md` | 版本对比 | No |
