...
2. **Discover structure** — `mindos_list_files` + targeted `mindos_search_notes`. Never assume top-level names.
...
| New file | `mindos_create_file` | Creating without checking siblings first |
| New Mind Space (zone + README + INSTRUCTION) | `mindos_create_space` | Using `create_file` only for `SomeSpace/README.md` when you mean a full Space |
